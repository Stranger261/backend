...
class LisPatient extends Model {
...
LisPatient.init(
  ...
  {
    sequelize,
    modelName: 'LisPatient',
    tableName: 'lis_patients',
    timestamps: true,
    createdAt: 'created_at',
    updatedAt: 'updated_at',
    indexes: [
      { name: 'idx_lis_patient_mrn', fields: ['mrn'], unique: true },
      { name: 'idx_lis_patient_name', fields: ['last_name', 'first_name'] },
      { name: 'idx_lis_patient_dob', fields: ['date_of_birth'] },
    ],
    hooks: {
      beforeCreate: async patient => {
        if (!patient.mrn) {
          patient.mrn = await LisPatient.generateMRN();
        }
      },
    },
  },
